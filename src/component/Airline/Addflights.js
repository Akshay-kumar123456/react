...
function ADDflights() {
  ...
  const handleaddflight = (e) => {
    ...
    axios
      .post(`http://localhost:8081/flight/add/${aid}`, data)
      .then((response) => {
  ...
  return (
    <div>
      <Navairline />
      <Row className="justify-content-end mb-3 no-gutters">
        <Col>
          <Button variant="primary" onClick={()=>navigate('/airline/addroute')} type="button" >
            Add Route
          </Button>
        </Col>
        <Col>
          <Button
            variant="warning"
            onClick={() => navigate("/airline/allroute")}
            style={{ width: "150px" }}
          >
            All Routes
          </Button>
  ...
  <p style={{ marginRight: '10px' }}>if routes didn't found then</p>
</div></center> */}
      </Container>
      {/* <Footer/> */}
    </div>
