...
function FlightSearch() {
  ...
  const [flightsAvailable, setFlightsAvailable] = useState(true);
  ...
  const searchFlights = async () => {
    try {
      ...
      if (response.data && response.data.length > 0) {
        setFlights(response.data);
        setFlightsAvailable(true); // Flights are available
      } else {
          ...
          <div className="row mb-8">
            <div className="col-md-3 mb-6">
              ...
              <Card style={{ width: "18rem" }}>
                ...
                <center>
        <Button variant="secondary" style={{margin:5}} onClick={resetFilters}>
          Reset Filters
        </Button>
      </center> 
              </Card>
